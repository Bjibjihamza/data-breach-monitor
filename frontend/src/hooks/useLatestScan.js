...
import { getLatestScan, getLatestScanDetections } from '../api/analytics.js';
...
export function useLatestScan(refreshKey) {
  const loader = useCallback(() => getLatestScan(), []);
  return useResource(loader, [refreshKey]);
}

export function useLatestScanDetections(filters = {}, refreshKey) {
  const params = useMemo(() => cleanFilters({ limit: 50, ...filters, offset: 0 }), [JSON.stringify(filters)]);
  ...
  useEffect(() => {
    ...
    getLatestScanDetections(params)
      .then((payload) => {
  ...
  const loadMore = useCallback(async () => {
    ...
    const nextOffset = Number(state.data.offset || 0) + Number(state.data.limit || params.limit || 50);
    setState((current) => ({ ...current, loadingMore: true, error: null }));
    try {
      const page = normalizePage(await getLatestScanDetections({ ...params, offset: nextOffset }));
      setState((current) => {
